...
namespace SystemBrightSpotBE.Dtos.Tenant
{
    public class CreateTenantDto : IValidatableObject
    {
        ...
        [Required(ErrorMessageResourceType = typeof(TenantResource), ErrorMessageResourceName = "StartDateRequired")]
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            if (start_date < today)
            {
                yield return new ValidationResult(TenantResource.StartDateMustBeTodayOrLater, new[] { nameof(start_date) });
            }
            ...
            var minEndDate = start_date.AddDays(7);
            if (end_date < minEndDate)
            {
                yield return new ValidationResult(TenantResource.EndDateMustBeAtLeast7DaysAfterStartDate, new[] { nameof(end_date) });
            }
        }
    }
}
